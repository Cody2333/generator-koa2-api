...
module.exports = class extends Generator {
  ...
  prompting() {
    ...
    this.log(yosay(
      'Welcome to ' + chalk.red('koa2-api') + ' generator!'
    ));
    return this.prompt([
      {
        type: 'input',
  ...
  writing() {
    this.fs.copyTpl(
      this.templatePath('koa2-api/src'),
      this.destinationPath(`${this.options.name}/src`)
    );
    this.fs.copyTpl(
      this.templatePath('koa2-api/test'),
      this.destinationPath(`${this.options.name}/test`)
    );
    this.fs.copyTpl(
      this.templatePath('koa2-api/.*'),
      this.destinationPath(`${this.options.name}`)
    );
    this.fs.copyTpl(
      this.templatePath('koa2-api/README.md'),
      this.destinationPath(`${this.options.name}/README.md`)
    );
    const pkg = this.fs.readJSON(this.templatePath('koa2-api/package.json'), {});
